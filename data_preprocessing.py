...
def get_stem_words(words, ignore_words):
    ...
    porter = PorterStemmer()
    for word in words:
        # Check if word is not in ignore_words
        if word not in ignore_words:
            # Lowercase the word and stem it
...
def create_bot_corpus(words, classes, pattern_word_tags_list, ignore_words):
    ...
    for intent in data['intents']:
        ...
        for pattern in intent['patterns']:  
            ...
            # Stem the words
            stemmed_pattern_words = [porter.stem(word.lower()) for word in pattern_words if word not in ignore_words]
            # add the tokenized words to the words list        
            words.extend(stemmed_pattern_words)
            # add the 'tokenized word list' along with the 'tag' to pattern_word_tags_list
            pattern_word_tags_list.append((stemmed_pattern_words, intent['tag']))
